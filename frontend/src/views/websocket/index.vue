<template>
  <a-spin :tip="'loading'" :spinning="loadLoading" class="spin-style">
    <div>
      <a-button class="ok-btn" block @click="showApplyResourceModal"> apply resource </a-button>
      <a-button class="ok-btn" block @click="showDeployModal"> deploy </a-button>
    </div>
  </a-spin>
  <apply-modal
    :title="'apply'"
  ...
  >
    <div style="margin-top: 32px">
      <a-form ref="applyState" :model="applyForm">
        ...
        <a-button class="ok-btn" block @click="showApplyTransModel"> apply </a-button>
      </a-form>
    </div>
  </apply-modal>
  <deploy-modal
  ...
  >
    <div style="margin-top: 32px">
      <a-form ref="applyState" :model="deployForm">
        ...
        <a-button class="ok-btn" block @click="deploy"> deploy </a-button>
      </a-form>
    </div>
  </deploy-modal>
  ...
  export default {
    ...
    setup() {
      ...
      const state = reactive({
        ...
        deployForm: {
          nodeEthereumUrl:
            'mainnet:https://eth-mainnet.alchemyapi.io/v2/wHl5-FZKD68DlhCuUiOXgYh4Z0q0L5fh', //graph_node 配置的以太坊节点
      ...
      const showApplyTransModel = async () => {
        ...
        let API = await api;
        let transaction = API.tx.resourceOrder.applyFreeResource(
          state.applyForm.cpu,
          state.applyForm.memory,
          state.applyForm.duration,
          state.applyForm.publicKey,
          1,
        );
      ...
      const apply = async (krp) => {
        ...
        API.tx.resourceOrder
          ...
          .signAndSend(krp, { nonce: -1 }, ({ status, events, dispatchError }) => {
            if (status.isInBlock) {
              if (dispatchError) {
                ...
                events.forEach(({ _phase, event: { data, method, _section } }) => {
      ...
      const deploy = () => {
        ...
        window.go.app.Deploy.DeployTheGraph(
          state.deployForm.nodeEthereumUrl,
          state.deployForm.ethereumUrl,
          state.deployForm.ethereumNetwork,
          state.deployForm.indexerAddress,
          state.deployForm.mnemonic,
        )
          .then(() => {
